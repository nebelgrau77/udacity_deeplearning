...
def get_data_loaders(
    ...
    data_transforms = {
        # resized and cropped, then some random color alternations (possible with photos taken
        # in various light conditions and with various cameras and camera settings)                
        "train": transforms.Compose([
            transforms.Resize(256),
            transforms.RandomRotation(10),
            transforms.CenterCrop(224),            
            transforms.ColorJitter(brightness=.2, contrast=.2, saturation=.2, hue=.05), 
            transforms.GaussianBlur(kernel_size=33, sigma=.7),            
            transforms.ToTensor(),
